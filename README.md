...
- `cartCounter: HTNLElement` - количество товаров в корзине.
...
- `CartCard` - карточка товаров в корзине.
...
- `inCart` - добавлен ли товар в корзину.
...
- `ContactsForm` - вторая форма с вводом почты и электронной почты.
...
1. После загрузке страницы обрабатывается событие `products:changed`и рендерятся карточки товаров.
...
4. После открытия корзины отображается список товарв и общая стоимость корзины с кнопокой оформления заказа, которая будет недоступна если корзина пуста.
