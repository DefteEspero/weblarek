...
8. При открытии корзины презентер открывает модальное окно и передаёт в него актуальный контейнер `BuyerCart`.
9. При событии `order:open` открывается первая форма оформления заказа `OrderForm`.
10. При изменении полей форм презентер передаёт новые значения в модель `Buyer`.
11. При событии `buyer:changed` обновляются формы `OrderForm` и `ContactsForm`.
12. При событии `order:submit` открывается вторая форма оформления заказа `ContactsForm`.
13. При событии `contacts:submit` презентер собирает данные заказа из моделей `Buyer` и `Cart`, а после успешного ответа показывает `OrderSuccess` с итоговой суммой.
14. После успешного оформления заказа очищаются данные покупателя и корзина.
15. При событии `success:close` закрывается модальное окно успешного заказа.
16. При событии `modal:close` закрывается текущее модальное окно.