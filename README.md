...
Назначение: промежэуточный базовый класс для карточек с изображением и категорией.
...
- `preview:button-click` - пользователь нажал на кнопку покупки или удаления из корзигы товара.
...
6. При событии `preview:button-click` презентер проверяет выбранный для просмотра товар в модели `Products`, если товар уже есть в корзине, удаляет его из модели `Cart`, иначе добавляет товар в модель `Cart`. После этого модальное окно закрывается через событие `modal:close`.
